...
controller.sendInvitation = async (req, res) => {
    ...
    try {
        ...
        let teamExits = await Users.findOne({ _id: team_user_id });
        ...
        if (playerExits) {
            ...
            await Users.updateMany({ _id: { $in: [team_user_id.toString(), player_id.toString()] } }, {
                "$push": {
...
controller.accepteInvitation = async (req, res) => {
    ...
    try {
        ...
        await Teams.updateOne({ _id: team_id, "players.player": new ObjectId(player_user_id) }, {
            "$set": {
                "players.$.status": 0
            },
        },);
